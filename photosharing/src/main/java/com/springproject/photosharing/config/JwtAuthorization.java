...
public class JwtAuthorization extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    response.addHeader(
      ...
      SecurityConstants.CLIENT_DOMAIN_URL
    );
    ...
    if (request.getMethod().equalsIgnoreCase("OPTIONS")) {
      ...
      if (
        jwtToken != null && !jwtToken.startsWith(SecurityConstants.TOKEN_PREFIX)
      ) {
      ...
      DecodedJWT jwt = JWT.decode(
        jwtToken.substring(SecurityConstants.TOKEN_PREFIX.length()) // decondig using the algo HMAC256 and removing the Bearer from token
      );
      String username = jwt.getSubject();
      List<String> roleArr = jwt.getClaims().get("roles").asList(String.class);
